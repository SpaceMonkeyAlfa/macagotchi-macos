...
**WARNING: Macagotchi for macOS relies on the Airport CLI utility. As of macOS Sonoma 14.4, Apple have deprecated Airport, without providing a good CLI alternative. Because of this, Macagotchi will not work on macOS >=14.4. As far as I can tell, the only way to access SSID WiFi data now is through Swift or C helper libraries. If anyone wants to fork this project, using their own swift or code to add back support, they are welcome to. This repo will remain up (archived), so that anyone who finds the rest of the code useful can use it for their own project. Macagotchi Pi and Windows are still functional at this point. **
